...
| `j`/`k`, `Enter`, `h`/`l` | with the edit panel open: select a preset card (`Down`/`Up` too), toggle it, lower / raise its **strength** (`Left`/`Right` too; `Shift` five steps) — the selected card also shows a strength slider; `Page Up`/`Page Down` change the image meanwhile |
