 ...
 * window's enhance state. The pure widget construction lives in enhance-ui.c
 * (enhance_ui_build_content); this thin wrapper owns the window-side glue --
 * storing the built widgets into p_win's fields and connecting each row's
 * "clicked" signal to _enhance_row_toggle, which owns the mask state. Split
 * out of _action_enhance to keep that under ~30 lines, mirroring
 * _move_build_box. */
static GtkWidget *
_enhance_build_box(GgazeWindow *p_win, const GPtrArray *p_presets) {
   ...
   GtkWidget       *p_box = enhance_ui_build_content(
      p_presets, c_basename, p_win->u_enhance_mask, b_previews, &ui);
   g_free(c_basename);
   /* Store the built widgets into the window fields enhance-ui does not own
    * (it holds no GgazeWindow state). */
   p_win->p_original_pic    = ui.p_original_pic;
   p_win->p_current_pic     = ui.p_current_pic;
   p_win->p_enhance_gallery = ui.p_gallery;
   p_win->p_enhance_scroll  = ui.p_scroll;
   for (guint i = 0; i < ui.u_n_presets; i++) {
      p_win->p_enhance_btns[i] = ui.p_btns[i];
      p_win->p_enhance_pics[i] = ui.p_pics[i];
   }
   /* Wire the row-toggle handler (window-side: it owns the mask + re-apply).
    * enhance-ui set each button's "idx" data (-1 Original, 0..n-1 presets),
    * which is what _enhance_row_toggle reads. */
...
_enhance_build_gallery_window(GgazeWindow *p_win, const GPtrArray *p_presets) {
   ...
   enhance_ui_apply_grid_columns(p_win->p_enhance_gallery != NULL
                                    ? GTK_FLOW_BOX(p_win->p_enhance_gallery)
                                    : NULL,
