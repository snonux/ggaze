 ...
 *
 * The focus grab is not cosmetic (5w0): this suite pops up the `!` run-script
 * popover, and a never-presented toplevel has no focus widget, which walks
 * GTK into an unguarded NULL on the X11 backend CI runs under. See
 * ggtest_focus_viewer() in tests/helpers/gtk_helpers.h for the full
 * mechanism. */
static GgazeWindow *
new_window(void) {
