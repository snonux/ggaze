...
test_shortcut_full_table_registered(void) {
   static const char *ACTIONS[] = {
      ...
      "win.quit",       "win.trash",       "win.delete",
      "win.undo",       "win.toggle-view", "win.mark",
      "win.mark-all",   "win.mark-range",  "win.copy",
      "win.shortcuts",  "win.zoom-in",     "win.zoom-out",
      "win.fullscreen", "win.slideshow",   "win.info",
      "win.back",       "win.enhance",     "win.enhance-save",
      "win.enhance-1",  "win.enhance-2",   "win.enhance-3",
      "win.enhance-4",  "win.enhance-5",   "win.enhance-6",
   ...
   /* The SHORTCUTS[] table has 37 rows now (some actions appear twice, e.g.
    * win.prev for h and Left; win.zoom-in for plus and equal). */
   g_assert_cmpint(g_list_model_get_n_items(G_LIST_MODEL(p_sc)), ==, 37);
   g_object_unref(p_sc);
   g_object_unref(p_win);
   drain_main(200);
}
