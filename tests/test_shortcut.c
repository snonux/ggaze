...
test_shortcut_full_table_registered(void) {
   static const char *ACTIONS[] = {
      ...
      "win.run-script",   "win.quit",       "win.trash",
   ...
   /* The SHORTCUTS[] table has 38 rows now (some actions appear twice, e.g.
    * win.prev for h and Left; win.zoom-in for plus and equal). */
   g_assert_cmpint(g_list_model_get_n_items(G_LIST_MODEL(p_sc)), ==, 38);
   g_object_unref(p_sc);
   g_object_unref(p_win);
   drain_main(200);
}
