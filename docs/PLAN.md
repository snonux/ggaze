...
| 45 | 2026-09-23 | **ICC colour management on the GEGL enhance/export path only** (xb2): with GEGL, a PNG/JPEG that embeds an ICC profile decodes through `gegl:png-load`/`gegl:jpg-load`, which tag the buffer with the embedded profile's babl space (sRGB when absent or unusable); the chain runs in that space (sRGB for CMYK/grey profiles), the preview asks babl for sRGB, `gegl:png-save`/`gegl:jpg-save` embed the source profile byte for byte, and a WebP export is `gegl:convert-space`d to sRGB. GEGL's loaders see a file only behind the loader's sniff, the dimension caps, `loader/intact.c` (complete container) and a bounding-box/extent check. Untagged files keep the loader path (pixels exactly as before); the plain view is unchanged (sRGB assumed). The `i` card names the colour space in every build (`icc.{c,h}`, plain C): the profile's `desc`, "sRGB (assumed)", "unreadable", or "not read for this format". | The enhance/export path already runs GEGL, so babl gives correct wide-gamut previews and lossless-profile exports without an lcms2 dependency; managing the fast view would cost decode speed and depends on what gdk-pixbuf/glycin already did. GEGL's loaders spin forever on a truncated file, hence the completeness gate. LUT profiles and non-PNG/JPEG profiles stay open (open-questions G). |
