...
| M9  | GEGL quick-enhance, crop/straighten/rotate (optional) | enhance done (tu0); crop/straighten/rotate done (wb2); ICC done (xb2) | `enhancer`, `a` side panel (layered presets) + hold-`Space` compare + non-destructive async preview + `s` save copy (no auto-save, prompt on navigate/trash/delete/move/open/quit) done; `c` crop / `R` straighten / `[` `]` rotate 90° tools on the same preview graph (`croprect`, `transform`, `tool-ctrl`; decision #35 order) done; ICC colour management on the enhance/export path (decision #45: GEGL ICC-aware PNG/JPEG decode, preview converted to sRGB, profile kept on PNG/JPEG export) + the `i` card's colour-space line done (xb2) |
...
| 45 | 2026-09-23 | **ICC colour management on the GEGL enhance/export path only** (xb2): with GEGL, a local PNG/JPEG whose embedded ICC profile babl parses to a non-sRGB space decodes through `gegl:png-load`/`gegl:jpg-load`, which tag the buffer with that space; the chain runs in it (sRGB for CMYK/grey profiles), the preview asks babl for sRGB, hold-`Space` shows the managed original (the file's identity chain through the same managed decode — CMYK/grey too — fetched lazily on the first press and dropped on discard/navigation, since it is a full-size texture outside the cache cap) rather than the unmanaged plain decode, `gegl:png-save`/`gegl:jpg-save` embed the space's profile — the source's own bytes when babl made the space from it, an earlier *equivalent* profile's bytes when babl matched it to that one's space (review 4: "byte for byte" was not true in general) — and a WebP export comes out sRGB. The managed path never gives a verdict of its own: it declines — and the loader path decodes or refuses exactly as before — any profile `icc_profile_is_sane` does not vouch for (review 3: babl reads tag data unchecked — a huge `curv` count segfaulted it or `babl_fatal` exited, from the `i` card too — so the header size, a tag table of ≤ 1024 entries, every tag inside the profile and every tag babl reads sized for what it reads are checked first; review 4: well-formed curves babl still crashes or aborts on are refused too — a `para` with a nonzero reserved word (babl reads it as a huge `curv`), a curve of more than 4096 points (babl copies every new space's profile out of a 64 KiB stack buffer), `para` types 3/4 with `d` or `c·d` outside [0, 0.998) (babl asserts), and `para` types 1/2 (babl reads a fifth parameter out of four); review 5: `para` parameters outside g, a ∈ (0, 10] and ±10 for the rest (babl names spaces after them: at −32767 its format names were cut short and its fish search spun forever) and any tone curve not shaped like one — falling, spanning under half the output range, flat over half its domain (babl aborted the JPEG export on constant / spiked / out-of-range curves)), a space babl built whose name is longer than its format names leave room for (254 − 1 − the longest encoding babl has registered, read once from its format table right after `gegl_init()` — review 8: babl walks it unlocked while GEGL workers insert formats — 229; review 7, was 220, which declined smooth type-4 curves on babl 0.1.112) or already another space's (every table curve is `lut-trc`, so a second grey or same-primaries table-curve profile would convert through the first one's formats; declined after babl kept it, slot charged), a space that does not convert through the profile's own formula curves (review 7: babl < 0.1.114, fedora:40's 0.1.112, keeps one `para` curve per type and gamma, so a second profile of the first one's gamma got the first one's curve — and its space; 64 even inputs per channel plus 1/1024 either side of each channel's own knee are read back through babl and compared with `icc_formula_curve_at`, 4e-4 linear + 0.1 % allowed — babl's own approximation of real formula curves is off by ≤ 3.1e-4 on 0.1.112 and 0.1.128 alike; review 8: review 7's five inputs and half an 8-bit step missed curves differing only between two knees and offsets under 0.002, up to ~16 and ~6 sRGB display steps near black; slot charged), a PNG whose iCCP libpng 1.6 would drop (intent ≥ 0xFFFF, a v4 length not a multiple of 4, over 8 000 000 bytes, a second iCCP (1.6.40 lets it replace the first, 1.6.58 drops it), one after PLTE, a bad CRC, …; review 5: `gegl:png-load` then builds a space from gAMA per file, outside the cap; 110 files filled babl's tables and crashed it) — including one beside an sRGB chunk (stricter than libpng: 1.6.40 drops the iCCP after an sRGB chunk but keeps it before one; review 7) or a gAMA / cHRM libpng 1.6.40 rejects (a duplicate, a gamma outside 16–625 000 000, chromaticities its fixed-point round trip fails; 1.6.40 then discards the iCCP), while a well-formed gAMA / cHRM beside a kept iCCP is accepted (review 6: `gegl:png-save`, GIMP and ImageMagick write them, and refusing them left ggaze's own PNG exports unmanaged on reload — sRGB on gdk-pixbuf 2.42; the kept iCCP wins in `gegl:png-load`, so no gAMA space is built), a profile babl declines outright (class, PCS, both CLUT directions, no curves or primaries: `icc_babl_kind`, never handed to babl), a CMYK profile LCMS cannot build babl's transform for (babl keeps a NULL one and crashes converting), a new profile past 16 per process that may grow babl's tables (its space / curve tables are fixed at 100, never freed, and a full one crashes; a profile costs a slot unless babl answered with a space it had and it carries no unused curve — a kTRC on RGB costs one; verdicts kept by SHA-256, the slot-free ones bounded at 64, oldest dropped; the `i` card and the render share the table, so whichever meets a profile first pays), anything the loader's sniff/caps or `loader/intact.c` do not vouch for (a truncated container; an oversized IHDR, checked before the inflate; corrupt PNG image data: CRCs, inflate, rows, one IDAT run; a JPEG libjpeg gives up on or that ends before its EOI, checked by a 1/8-scale libjpeg pass with EOF fatal because `gegl:jpg-load` exits the process on libjpeg errors and on a cut file; every JPEG in a build without libjpeg), a profile for another number of colour components than the image's, a decode whose extent differs from the header, a non-local file, and an sRGB-profiled or untagged file. Visible change for every file on the enhance path: main's enhancer copied the loader's texture as premultiplied B8G8R8A8 into an RGBA buffer (red and blue swapped); xb2 reads it as R8G8B8A8, so untagged files now enhance/export with their true colours. A cancelled load returns CANCELLED without the loader's decode, and the managed original never falls back to a plain decode. The `i` card names the colour space in every build (`icc.{c,h}`, plain C): the profile's `desc` (one line, ≤64 chars) with "may be managed on enhance/export" only when the enhancer's own gates pass header-deep (`enhancer_would_manage`, size caps included; the whole-file checks cost ~150 ms a file, too slow for the card), "sRGB (assumed)", "unreadable", or "not read for this format". | The enhance/export path already runs GEGL, so babl gives correct wide-gamut previews and lossless-profile exports without ggaze doing any colour maths itself (lcms2, babl's own dependency, is used only to vet a CMYK profile before babl keeps a broken transform); managing the fast view would cost decode speed and depends on what gdk-pixbuf/glycin already did. GEGL's loaders fail badly where the loader fails cleanly (spin on EOF, silent black PNG, process exit on a libjpeg error), hence "vouch or decline". The file-swap window between the checks and GEGL's own open is documented, not closed (GEGL takes a path; it is also the one way an unvetted profile or gAMA reaches babl, outside the cap), as are two per-decode upstream leaks the LSan suppressions hide (babl's grey branch: 4 × the profile per grey decode; `gegl:jpg-load`: 4 KiB per JPEG decode; docs/gegl.md "Leaks we cannot fix"). LUT-only RGB profiles and non-PNG/JPEG profiles stay open (open-questions G). |
