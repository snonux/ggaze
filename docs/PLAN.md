...
| 36 | 2026-07-12 | **Enhance presets** (was X+#4): superseded by implementation (tu0) — presets are **layered** (any number toggle on/off independently and compose in the preview graph), not a single active/replace preset; `0`/"Original" reset clears all. Curves via `gegl:contrast-curve`; no fine-adjust nudging in v1. Crop/straighten/rotate stack on top since wb2 (decision #44). | Layering turned out more useful than single-replace for comparing combinations; recorded here so the decision log matches what actually shipped. |
