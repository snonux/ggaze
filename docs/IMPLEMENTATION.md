...
- Target the **plain-C modules** (`detect`, `navigator`, `thumbnail`,
  ...
  `preview-scale`, `logical-size`, and in the GEGL lane `loader/intact`).
  No GTK display needed.
...
- Unit: `test_enhancer.c` (gated): each preset dims + non-zero; export file
  ...
  default, a removed preset dropped and a new row off. 8l2 (decision #53):
  `test_enhancer_preview.c` (gated) — a full-size source renders the export
  chain's pixels exactly (turn, crop, Sharpen) and the full-resolution
  chain is the plain GEGL graph (an independent reference: no scaled
  radius leaks into the export); a capped source is the image at a
  quarter, its render of a crop stands for the export's size and keeps
  the crop's place, its own texture stands for the original; a source
  from the viewer's decode starts no loader decode and holds the same
  pixels; a Gaussian blur's edge on a quarter-size source is as wide in
  image pixels as at full size; a managed file's scaled source is the
  managed decode (swapped.png: blue); which views a source serves; the
  cards from the source at 128 px and a cancelled batch; an undecodable
  file fails the build; the delay / failure seams.
  `test_preview_scale.c` and `test_logical_size.c` (every lane): the
  source scale from the view (fit x device scale x 1.5, never up, the
  512 / 4096 px bounds, a cap, the default view), the covers rule, the
  card scale, the pixel-length table; a texture's logical size set, read,
  removed and per object. `test_transform.c`: `transform_scale` carries
  the crop base to base (also after a turn) and a straightened crop's
  output to a pixel; `test_viewer.c`: a quarter-size texture standing for
  the image keeps its fit, rectangle and 100 %. xb2 (ICC): `test_enhancer_icc.c` (gated) — a
