...
- `src/viewer.c` — `GtkGestureZoom` (pinch about the midpoint, panning
  ...
  rest of it; `src/window.c` routes `toggle-info` to
  `win.info` and stops a slideshow on `navigate`. The GTK handlers are
...
- Integration: `test_gestures.c` (see the table above) and five
  ...
  two-finger tap in the crop tool leaves it as it was).
