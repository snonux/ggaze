...
| `test_gestures.c` | M1 (zb2) | Touch on a presented window, driven through the viewer's public gesture bodies (GTK4 cannot synthesise touch) and the controllers' own signals: a pinch zooms about its midpoint (the image pixel under it stays put, also when the midpoint moves) with the wheel's clamp and NaN / Inf / zero / negative-scale / non-finite-midpoint guards; the zoom / swipe controllers are installed and wired (swipe touch-only; a cancelled tap-shaped touch toggles nothing); a pinch ends a one-finger tool drag (END where the finger was) and a pan drag, swallowing the rest; a two-finger tap toggles the info card on and off and leaves a fitted view fitted, a moving / pinching / > 250 ms one does not; a leftward swipe shows the next file and a rightward one the previous, while vertical, tiny and zero-velocity swipes, a swipe with a tool overlay installed and one over a picture zoomed wider than the window navigate nowhere (zoomed but narrower: it does). `test_enhance_flow.c` adds the GEGL halves: a swipe away from a dirty preview prompts (Cancel stays, Discard moves) and the crop tool refuses a swipe until `Esc`. |
...
- `src/viewer.c` — `GtkGestureZoom` (pinch about the midpoint),
  touch-only `GtkGestureSwipe` (`navigate`), two-finger tap
  (`toggle-info`); `src/window.c` routes `toggle-info` to `win.info`.
- Unit: `test_gesture_math.c` — zoom keeps the point's pixel, clamps
  (incl. the fit-raised ceiling), NaN/Inf anywhere refused; pinch refuses
  zero / negative / non-finite scale and overflow; swipe refuses
...
- Integration: `test_gestures.c` (see the table above) and two
  `test_enhance_flow.c` subtests (dirty-gated swipe, crop tool refuses).
