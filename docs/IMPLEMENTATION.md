...
per-image history of whole edit snapshots (`src/edit-history.{c,h}`). ICC
...
- Unit: `test_enhancer.c` (gated): each preset dims + non-zero; export file
  ...
  its start dropped, clear, and every entry point's NULL argument. xb2 (ICC): `test_enhancer_icc.c` (gated) — a
