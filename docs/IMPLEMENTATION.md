...
- Unit: `test_enhancer.c` (gated): each preset dims + non-zero; export file
  ...
  canonical values (seven steps up and down land on the parsed default),
  the card / title label (signed when the range goes below zero), every
  malformed placeholder refused with its message, and the same text under
  ...
  with the parse message. xb2 (ICC): `test_enhancer_icc.c` (gated) — a
