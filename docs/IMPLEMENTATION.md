...
- `src/viewer.c` — `GtkGestureZoom` (pinch about the midpoint, panning
  ...
  `GGAZE_VIEWER_DRAG_CANCEL`; `src/window.c` routes `toggle-info` to
...
- Integration: `test_gestures.c` (see the table above) and four
  `test_enhance_flow.c` subtests (dirty-gated swipe, crop tool refuses, a
  pinch in the straighten tool levels nothing, a pinch in the crop tool
  keeps the dragged rectangle).
- **Known test gap** (needs real touch sequences, which GTK 4 cannot
  ...
  signals do run every handler: GtkGestureZoom "begin" / "scale-changed"
  / "cancel" / "end" (no points: the widget centre), GtkGestureSwipe
  "begin" / "swipe", the drag gesture and the scroll controller.
