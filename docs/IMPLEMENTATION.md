...
- Animated GIF/WebP playback (task yb2, decision #46) — done. Not a
  ...
  for them and plays them: a tick callback on the frame clock picks the
  frame that is due and draws it at the first frame's geometry, only while
  mapped and the frame clock runs, restarting from the first frame on every
  `set_texture`, holding it while a crop / straighten tool is up. So the
  ...
  frames + canvas) and a **playback budget** (`animation_within_budget`:
  frames × canvas ≤ the still cap, canvas ≤ 4 Mi pixels, ≤ 1000 frames);
  anything else goes down the still path byte-for-byte as before. Frame
  delays are clamped to `GGAZE_ANIM_MIN_DELAY_MS` (20 ms).
...
- Unit (yb2): `test_animation.c` (TAP) — the probe over `anim.gif` /
  ...
  static decode, clamped zero delays).
  ...
  exactly webp-pixbuf-loader 0.2.7's two messages),
  `/scaled_animation_is_first_frame`.
...
- Integration (yb2): `test_viewer.c` `/viewer/animation_plays_and_keeps_
  ...
  `/zero_delay_plays_at_the_clamp` (frame changes over one second ≤
  W / 20 ms + 2, and at least one), `/unmapped_viewer_holds_first_frame`.
